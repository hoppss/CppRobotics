...
int main() {
  ...
  for (unsigned int i = 1; i < r_x.size(); i++) {
    cv::line(bg, cv_offset(r_x[i - 1], r_y[i - 1], bg.cols, bg.rows), cv_offset(r_x[i], r_y[i], bg.cols, bg.rows), cv::Scalar(0, 0, 0), 10);
  }
  for (unsigned int i = 0; i < x.size(); i++) {
    cv::circle(bg, cv_offset(x[i], y[i], bg.cols, bg.rows), 30, cv::Scalar(255, 0, 0), 5);
  }
